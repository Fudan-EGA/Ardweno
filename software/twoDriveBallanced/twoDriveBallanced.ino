...
/************PID参数****************/
//角度环
float ANG_P_DATA = 300.00f; // 比例常数 Proportional Const
float ANG_I_DATA = 6.00f;  // 积分常数  Integral Const    --动态响应
float ANG_D_DATA = 280.00f; // 微分常数 Derivative Const    --预测作用，增大阻尼
...
float TARGET_SPEED = 0.00f;   //设定目标点初值
...
void loop() {
  ...
  if (Serial.available()) { //串口接收控制
    switch(Serial.read()){
      case 'T': control_flag=STOP;break;
  ...
  switch(control_flag){
    ...
    case (BALLANCE|CHANGE):
      ...
      */
      ...
      sendStringBT("\r\n");
      sendStringBT("PL:");
      sendFloatBT(pid_spdL.Proportion);
      sendStringBT("\r\n");

      sendStringBT("PR:");
      sendFloatBT(pid_spdR.Proportion);
      sendStringBT("\r\n");


      sendStringBT("\r\n");
      sendStringBT("IL:");
      sendFloatBT(pid_spdL.Integral);
      sendStringBT("\r\n");

      sendStringBT("IR:");
      sendFloatBT(pid_spdR.Integral);
      sendStringBT("\r\n");


      sendStringBT("\r\n");
      sendStringBT("DL:");
      sendFloatBT(pid_spdL.Derivative);
      sendStringBT("\r\n");

      sendStringBT("DR:");
      sendFloatBT(pid_spdR.Derivative);
      sendStringBT("\r\n");

      control_flag=STOP;
      break;
    case (CHANGE|FORWARD):
      PID_SetSpeed(&pid_spdL, TARGET_SPEED);
      PID_SetSpeed(&pid_spdR, TARGET_SPEED);
      sendStringBT("SPD:");
      sendFloatBT(TARGET_SPEED);
      sendStringBT("\r\n");
